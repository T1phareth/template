...
ll n,m,k,sta[N],top,q,totA,totB,totC;
...
void Minkowski(Node *C1,Node *C2,ll n,ll m,Node *A,ll &tot)
{
...
int main()
{
	...
	for(ll i=1;i<=k;i++)
		cin >> C3[i].x >> C3[i].y;
	Convex(C3,k);
	Minkowski(C1,C2,n,m,A,totA),Convex(A,totA);
	Minkowski(A,C3,totA,k,A,totA),Convex(A,totA);
	A[0]=A[1];for(ll i=totA;i>=1;i--)A[i]=A[i]-A[0];
	cin >> q;
	while(q--)
	{
		Node tmp;cin >> tmp.x >> tmp.y;
		cout << (in(tmp*3-A[0],A,totA)?"YES":"NO") << endl;
	}
	return 0;
}
